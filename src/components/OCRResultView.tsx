...
export default function OCRResultView({ result, onClose, onRetry }: OCRResultViewProps) {
  ...
  const confidenceColor = 
    result.confidence >= 80 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
    result.confidence >= 60 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' :
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden max-w-2xl mx-auto">
      ...
      <div className="p-6 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-orange-800 dark:text-orange-300">
                Le texte a été extrait automatiquement et peut contenir des erreurs. Vérifiez toujours les informations sur l'emballage original.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-semibold ${confidenceColor}`}>
        ...
        <div className="space-y-4">
          {activeTab === 'sections' && result.sections ? (
            <>
              ...
              {!result.sections.ingredients && !result.sections.allergens && !result.sections.legalMentions && (
