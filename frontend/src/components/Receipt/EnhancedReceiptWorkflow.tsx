...
import { ReceiptScannerWithMode } from './ReceiptScannerWithMode';
import { ReceiptLineReviewEnhanced } from './ReceiptLineReviewEnhanced';
import { ReceiptValidation } from './ReceiptValidation';
import { parseReceiptLines, filterProductLines } from './services/receiptLineParser';
import type { ReceiptData } from './types';
import type { ReceiptLine } from '../../types/receiptLine';

type EnhancedReceiptWorkflowProps = {
  territory: string;
...
/**
 * Enhanced Receipt Workflow with integrated modules:
 * - Single/Multi-photo capture
 * - Auto-crop and image enhancement (Module H)
 * - Line-by-line OCR review (Module I)
 * - Territory and anomaly detection (Modules F & G)
 */
export const EnhancedReceiptWorkflow: React.FC<EnhancedReceiptWorkflowProps> = ({
  territory,
  onSubmit,
}) => {
  ...
  const handleScanComplete = (text: string, images: string | string[]) => {
    setExtractedText(text);
    setImageData(images);

    // Parse OCR text into lines for review
    const parsedLines = parseReceiptLines(text);
    const filteredLines = filterProductLines(parsedLines);

    setReceiptLines(filteredLines);
    setWorkflowState('review');
  };

  const handleLinesConfirmed = (validatedLines: ReceiptLine[]) => {
    // Reconstruct text from validated lines for the validation component
    const reconstructedText = validatedLines
      .map((line) => line.raw)
      .join('\n');

    setExtractedText(reconstructedText);
    setReceiptLines(validatedLines);
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`flex-1 text-center ${
              workflowState === 'review' ? 'font-bold text-blue-600' : 'text-gray-500'
            }`}
          >
            <div
              className={`mx-auto w-10 h-10 rounded-full flex items-center justify-center mb-2 ${
                workflowState === 'review' ? 'bg-blue-600 text-white' : 'bg-gray-300'
              }`}
            >
              2
            </div>
            <p className="text-sm">📊 Révision</p>
          </div>
          <div className="flex-1 h-1 bg-gray-300">
            <div
              className={`h-full transition-all duration-500 ${
          ...
          <div className="flex-1 h-1 bg-gray-300">
            <div
              className={`h-full transition-all duration-500 ${
                workflowState === 'complete' ? 'bg-green-600' : 'bg-gray-300'
              }`}
            ></div>
          </div>
          <div
      ...
      {workflowState === 'scan' && (
        <ReceiptScannerWithMode onScanComplete={handleScanComplete} territory={territory} />
      )}

      {workflowState === 'review' && (
        <ReceiptLineReviewEnhanced
          lines={receiptLines}
          onConfirm={handleLinesConfirmed}
      ...
      {workflowState === 'validate' && (
        <ReceiptValidation
          ...
          imageData={Array.isArray(imageData) ? imageData[0] : imageData}
          territory={territory}
        ...
        <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
          ...
          <div className="bg-yellow-50 p-4 rounded border-l-4 border-yellow-500 mb-6">
            <p className="text-sm text-yellow-900">
              ...
              <br />
              Cette observation a été validée avec le système de pré-validation.
              <br />
              Toutes les lignes ont été vérifiées manuellement pour garantir la qualité.
              <br />
              Compatible avec les modules Territoires (F) et Anomalies (G).
            </p>
          </div>
          ...
          <div className="mt-6 p-3 bg-gray-100 rounded text-sm text-gray-700">
            <p>
              ...
              <br />
              OCR local avec auto-recadrage (Module H) + Pré-validation humaine (Module I)
